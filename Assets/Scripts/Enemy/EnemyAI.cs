...
public class EnemyAI : MonoBehaviour
{
  ...
  //  [SerializeField] private Animator _animator;
    ...
    public EEnemyState _state = EEnemyState.Patrolling; // Для хранение текущего состояния бота - по умолчанию - патруль
    //
    ...
    private void Start()
    {
        ...
        // Устанавливаем максимально время ожидания для проверки на застревание
    ...
    /// <summary>
    ///Решаем проблему с застреванием ботов
    /// </summary>
    private void CheackJamming() 
    {
        // Мы не проверяем на застревание при приследовании (хотя возможно надо переходить в режим поиска)
        if (_state == EEnemyState.Chasing) return;
        if (Time.time - _currentTimejamming > _timeTojamming)
        {
            // Если бот застрял, то переводим его в патрулирование
            if (Vector3.Distance(_oldPosition,_agent.transform.position)<0.05 && _oldState==_state)
            {
                Debug.Log(gameObject.name + " застрял!");
                StartPatrol(); 
            }
            // Записываем новые данные отслеживания
            _oldPosition= _agent.transform.position;
            _currentTimejamming = Time.time;
        }
    ...
    {        
        ...
        /// Это действия, при нахождении игрока в зоне атаки. Нужно добавить на проверку припятсия между игроком и врагом (скорее всего луч)
        if (_state == EEnemyState.Chasing && Vector3.Distance(transform.position, GameMode.FirstPersonMovement.transform.position) < 0.6 && isVisiblePlayer)
        {
            ...
            {
                // Смерть монстра только после удара - привезать к анимации
